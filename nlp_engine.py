...
COLLOQUIAL_BODY_PATTERNS: list[tuple[re.Pattern, str]] = [
    ...
    (
        re.compile(
            r"\b(?:my\s+)?(?:balls?|testicles?|nuts?|scrotum)\s*" + _PAIN_VERBS, re.I
        ),
        "Orchialgia / testicular pain",
    ),
    (
        re.compile(
            r"\b(?:my\s+)?(?:balls?|testicles?|nuts?)\s*" + _SWELLING_VERBS, re.I
        ),
    ...
    (
        re.compile(
            r"\b(?:my\s+)?(?:dick|penis|cock|willy|todger|member)\s*" + _PAIN_VERBS,
            re.I,
        ),
        "Penile pain",
    ),
    ...
    (
        re.compile(
            r"\b(?:my\s+)?(?:vagina|vulva|vag|cooch|privates?)\s*" + _PAIN_VERBS, re.I
        ),
        "Vaginal / vulvar pain",
    ),
    (
        re.compile(
            r"\b(?:down\s+there|down\s+below|in\s+my\s+(?:private\s+area|groin))\s*"
            + _PAIN_VERBS,
            re.I,
        ),
        "Genital / pelvic pain",
    ...
    (
        re.compile(
            r"\b(?:my\s+)?(?:stomach|tummy|belly|gut|abdomen)\s*" + _PAIN_VERBS, re.I
        ),
        "Abdominal pain",
    ),
    (
        re.compile(
            r"\b(?:my\s+)?(?:stomach|tummy|belly)\s*(?:is\s+)?(?:bloated|swollen|distended|big|huge)",
            re.I,
        ),
    ...
    (
        re.compile(
            r"\b(?:my\s+)?(?:back|spine|lower back|upper back|lumbar)\s*" + _PAIN_VERBS,
            re.I,
        ),
        "Dorsalgia",
    ),
    ...
    (
        re.compile(
            r"\b(?:my\s+)?(?:chest|breast bone|sternum|ribs?)\s*" + _PAIN_VERBS, re.I
        ),
        "Chest pain",
    ),
    # Head
    (re.compile(r"\b(?:my\s+)?(?:head|skull)\s*" + _PAIN_VERBS, re.I), "Cephalgia"),
    # Neck
    (
    ...
    # Joints (general)
    (re.compile(r"\b(?:my\s+)?(?:knee|knees)\s*" + _PAIN_VERBS, re.I), "Knee pain"),
    (re.compile(r"\b(?:my\s+)?(?:hip|hips)\s*" + _PAIN_VERBS, re.I), "Hip pain"),
    (
        re.compile(r"\b(?:my\s+)?(?:shoulder|shoulders)\s*" + _PAIN_VERBS, re.I),
        "Shoulder pain",
    ),
    (re.compile(r"\b(?:my\s+)?(?:ankle|ankles)\s*" + _PAIN_VERBS, re.I), "Ankle pain"),
    (re.compile(r"\b(?:my\s+)?(?:wrist|wrists)\s*" + _PAIN_VERBS, re.I), "Wrist pain"),
    (
    ...
    (
        re.compile(
            r"\b(?:my\s+)?(?:finger|fingers|thumb|thumbs)\s*" + _PAIN_VERBS, re.I
        ),
        "Hand / finger pain",
    ),
    # Urination
    (
        re.compile(
            r"\b(?:it\s+)?(?:hurts?|burns?|stings?)\s+when\s+(?:i\s+)?(?:pee|urinate|go\s+to\s+the\s+bathroom)",
            re.I,
        ),
        "Dysuria",
    ),
    (
        re.compile(
            r"\b(?:burning|pain|discomfort)\s+(?:when|while|during)\s+(?:peeing|urinating|urination)",
            re.I,
        ),
        "Dysuria",
    ),
    ...
    (
        re.compile(
            r"\b(?:it\s+)?(?:hurts?|burns?|stings?|bleeds?)\s+when\s+(?:i\s+)?(?:poop|defecate|go\s+to\s+the\s+toilet|have\s+a\s+bowel\s+movement)",
            re.I,
        ),
    ...
    (
        re.compile(
            r"\bblood\s+(?:when|after)\s+(?:i\s+)?(?:wipe|poop|go\s+to\s+the\s+toilet)",
            re.I,
        ),
    ...
    (
        re.compile(
            r"\b(?:can't|cannot|hard to|trouble|difficulty)\s+(?:breathe|breathing|catch\s+(?:my\s+)?breath)",
            re.I,
        ),
        "Dyspnea",
    ),
    (
        re.compile(
            r"\b(?:get|gets?|getting)\s+(?:out\s+of\s+breath|winded|breathless)\s+(?:easily|quickly|fast)",
            re.I,
        ),
        "Exertional dyspnea",
    ),
    ...
    (
        re.compile(
            r"\b(?:can't|cannot|hard to|trouble|difficulty)\s+(?:swallow|eat|keep\s+food\s+down)",
            re.I,
        ),
        "Dysphagia / nausea",
    ),
    ...
    (
        re.compile(
            r"\b(?:can't|cannot)\s+(?:sleep|fall\s+asleep|stay\s+asleep|get\s+to\s+sleep)",
            re.I,
        ),
        "Insomnia",
    ),
    (
        re.compile(
            r"\b(?:wak(?:e|ing)\s+up\s+(?:in\s+the\s+night|at\s+night|throughout\s+the\s+night))",
            re.I,
        ),
    ...
    (
        re.compile(
            r"\b(?:found|feel|noticed|there'?s?)\s+(?:a\s+)?(?:lump|bump|mass|nodule|growth)\s+(?:in|on|near|under)\s+(?:my\s+)?\w+",
            re.I,
        ),
    ...
    (
        re.compile(
            r"\b(?:my\s+)?(?:glands?|lymph\s+nodes?)\s+(?:are\s+)?(?:swollen|up|enlarged)",
            re.I,
        ),
        "Lymphadenopathy",
    ),
    (re.compile(r"\bswollen\s+glands?\b", re.I), "Lymphadenopathy"),
]
...
TEMPORAL_PATTERNS = [
    ...
    (
        r"(since|for the (last|past))\s+(\d+\s*(day|days|week|weeks|month|months|year|years))",
        "duration",
    ),
    (r"(started|began|onset)\s+(yesterday|today|last\s+\w+|\d+\s*\w+\s*ago)", "onset"),
    (
        r"(gradual|sudden|acute|chronic|intermittent|persistent|constant|occasional)",
        "character",
    ),
]
...
VITAL_PATTERNS = [
    ...
    (
        ...
        if 60 <= int(m.group(1)) <= 250 and 40 <= int(m.group(2)) <= 150
        else None,
    ),
    # Heart rate / pulse
    (
    ...
    (
        re.compile(
            r"(?:o2 sat(?:uration)?|spo2|oxygen saturation|sats?)[:\s]+([\d.]+)\s*%?",
            re.I,
        ),
    ...
    (
        re.compile(
            r"(?:glucose|blood sugar|bs|bg|cbg|fbs)[:\s]+([\d.]+)\s*(?:mg/dl|mmol)?",
            re.I,
        ),
...
SOCIAL_PATTERNS = {
    "smoking": re.compile(
        r"\b(smok(?:es?|ing|er)|cigarette|tobacco|pack(?:\s+a\s+|\s+per\s+)(?:day|week))\b",
        re.I,
    ),
    "alcohol": re.compile(
    ...
    "exercise": re.compile(
        r"\b(exercis(?:e|ing|es)|workout|gym|walk(?:ing)?|run(?:ning)?|sedentary|active)\b",
        re.I,
    ),
...
class MedicalNLPEngine:
    ...
    def _extract_character(self, context: str) -> Optional[str]:
        found = [
            c
            for c in PAIN_CHARACTERS
            if re.search(r"\b" + re.escape(c) + r"\b", context, re.I)
        ]
    ...
    def _extract_location(self, context: str) -> Optional[str]:
        found = [
            r
            for r in ANATOMICAL_REGIONS
            if re.search(r"\b" + re.escape(r) + r"\b", context, re.I)
        ]
    ...
    def extract_medications(self, text: str) -> list[Medication]:
        ...
        for m in self._med_re.finditer(text):
            ...
            if any(
                w in ctx_lower
                for w in ["prescribed", "started on", "start", "initiate", "begin"]
            ):
                status = "prescribed"
            elif any(
                w in ctx_lower
                for w in ["currently taking", "currently on", "takes", "on "]
            ):
                status = "current"
    ...
    def extract_diagnoses(self, text: str) -> list[Diagnosis]:
        ...
        dx_patterns = [
            re.compile(
                r"(?:diagnos(?:ed?|is)|impression|assessment|working diagnosis)[:\s]+([^.\n]+)",
                re.I,
            ),
            re.compile(
                r"(?:consistent with|likely|probable|confirmed|ruling out)\s+([^.,\n]+)",
                re.I,
            ),
            ...
            for m in pat.finditer(text):
                ...
                for condition, (code, desc) in ICD10_MAP.items():
                    if condition in fragment and condition not in seen:
                        ...
                        certainty = "confirmed"
                        if re.search(
                            r"rule out|possible|probable|likely|r/o", m.group(0), re.I
                        ):
                            certainty = "possible"
